...
<div class="row product-list">
  ...
  <aside class="col-md-2">
    <form action="${pageContext.request.contextPath }/admin/product/search" accept-charset="UTF-8" method="get"><input name="utf8" type="hidden" value="✓">
  <div>
...
<div class="pagination">
      <ul class="pagination">
			<c:set value="${pageContext.request.contextPath }/admin/product/index/${page-1 }"  var="urlPre"></c:set>
                <c:set value="${pageContext.request.contextPath }/admin/product/index/${page+1 }" var="urlNext"></c:set>
                   <c:if test="${(page-1) > 0 }">
                    ...
                    <c:choose>
                    	<c:when test="${sumPage < 4 }">
                    		<c:forEach begin="1"  end="${sumPage }" var="i">
                      	 		<li><a <c:if test="${i == page }">class=active</c:if> href="${pageContext.request.contextPath }/admin/product/index/${i }">${i } <span class="sr-only"></span></a></li>
                      		</c:forEach>
                    	</c:when>
